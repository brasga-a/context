...
- The `Stop`-hook changelog reminder diffs against `git status`, which needs a commit baseline to
  ...
  `CLAUDE.md`.
- **Archival note**: the first archive attempt hit a WSL2 DrvFs/9p filesystem glitch — the
  directory produced by `mv openspec/changes/add-claude-workflow openspec/changes/archive/2026-07-23-add-claude-workflow`
  became unreadable immediately after the rename (parent listing showed the name, but open/readdir/
  stat on it all failed, consistent with a stale 9p file handle; `dmesg` showed a
  `p9io.cpp AcceptAsync` "Operation canceled" event). The directory itself couldn't be removed
  either (`rmdir` reported "Directory not empty" while nothing inside could be read). No content
  was lost — every artifact was reconstructed from this session's context into this directory
  (`2026-07-23-add-claude-workflow-2`) instead. The orphaned `2026-07-23-add-claude-workflow`
  directory likely needs a WSL restart (`wsl --shutdown` from Windows, then reopen) before it can
  be inspected or removed normally.
