...
- [x] 1.2 Run `cargo build --workspace` and `cargo test --workspace` from the repo root; confirm
      both crates and the placeholder binary all build/test — also fixed a pre-existing incomplete
      rename (`notes-lexer` → `context-lexer`) that blocked the build once the workspace was wired
      up
