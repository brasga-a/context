...
Defines the Claude Code developer workflow for this repository: how the cargo workspace is
structured, the `/check` command for running the dev loop, automation that keeps changelogs and
formatting current, protection for generated files, and the project context documented in
`CLAUDE.md`.
